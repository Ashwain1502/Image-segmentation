# Intoduction
This project uses a min-cut max-flow algorithm to extract the foreground from the background in an image in Python. Then shows the object selected as result.

## Functions
A Python GUI application that can take an image as input and can be used to get foreground from background.

## Steps to Run 
1. Select the foreground area with red outline.
2. Select the background area with blue outline.
3. Press Enter to run and extract the results.

P.S. The bettere and precise marking in earlier steps leads to better extraction of the object.

## Example 
<img alt="Picture" title="Step 1" src="/Images/pic.png" width="300px" height="250px">

> ## Contributors
> Ashwani Kumar (ashwink1502@gmail.com).
